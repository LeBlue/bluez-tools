...
B<-n, --non-interactive>
    Run the agent in the foreground, but non-interactive, does not ask for input of passkeys or authorization, if not covered by the PIN's file.
    If the flag is not given, the agent will rely on a pin file and request manual user authorization.
